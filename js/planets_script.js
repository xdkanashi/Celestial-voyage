document.addEventListener('DOMContentLoaded', function() {
    ...
    const planetStat = document.querySelector('.planet-stat');
    const planetText = document.querySelector('.planet-text');
    const planet2Text = document.querySelector('.planet2-text');
    ...
    planetStat.style.opacity = '0'; // Устанавливаем начальную прозрачность для .planet-stat
    planetStat.style.display = 'none'; // Скрываем .planet-stat
    ...
    openButton.addEventListener('click', function() {
        ...
        // Плавно показать .planet-stat после нажатия кнопки
        planetStat.style.display = 'block';
        setTimeout(() => {
            planetStat.style.opacity = '1';
        }, 100);
    });
    ...
    displayLines(firstHalf, planetText);
    displayLines(secondHalf, planet2Text);
    
    // Начальное состояние: скрываем `text-bar` элементы
    planetText.style.display = 'none';
    planet2Text.style.display = 'none';

    // Обработчик клика на первом `text-bar` элементе
    planetText.addEventListener('click', function() {
        planetText.style.display = 'block';
    });

    // Обработчик клика на втором `text-bar` элементе
    planet2Text.addEventListener('click', function() {
        planet2Text.style.display = 'block';
    });
    ...
    const elementsToClick = [openButton, planetText];
    ...
    // Добавляем обработчик клика на planetText
    planetText.addEventListener('click', function() {
        planet2Text.style.display = 'block';
        setTimeout(() => {
            planet2Text.style.opacity = '1';
            planetText.style.opacity = '0'; // Скрыть planetText
        }, 100);
    });
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    buttons.forEach(button => {
        button.addEventListener('click', function() {
            ...
            fetchFilteredPlanets(filter);
        });
    });
});

function fetchFilteredPlanets(filter) {
    fetch('filter.php', {
        method: 'POST',
        headers: {
